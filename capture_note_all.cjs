...
(async () => {
  ...
  page.on('request', req => {
    ...
    if (method === 'POST' || method === 'PUT') {
      const url = req.url();
      if (!url.includes('sentry') && !url.includes('google-analytics') && !url.includes('doubleclick') && !url.includes('cloudflareinsights') && !url.includes('reader/feed')) {
         console.log(`\n[REQUEST] ${method} ${url}`);
  ...
  if (input) {
     console.log('Found placeholder, clicking...');
     ...
     if(bb) {
        ...
        if (ed) {
           ...
           await ed.type('This is a test note to grab the API.');
