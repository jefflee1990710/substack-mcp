...
An MCP (Model Context Protocol) server exposing 24 tools for automating a Substack publication (drafts, posts, notes, tags, profile/publication settings, stats, reader feed/threads). Single-package ESM Node.js, plain JavaScript — no TypeScript, no build step, no tests, no linter.
